    ...
    try {
        ...
        for (const item of items) {
            ...
            if (competition.status !== "active") {
                return error(c, ErrorCodes.VALIDATION_ERROR, `Competition "${competition.title}" is not active`, 400);
            }
            const available = competition.maxTickets - (competition.ticketsSold || 0);
            if (item.quantity > available) {
                return error(c, ErrorCodes.VALIDATION_ERROR, `Only ${available} tickets available for "${competition.title}"`, 400);
            }
        }
